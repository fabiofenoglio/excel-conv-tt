...
func GroupByRoom(rows []ParsedRow) []GroupedRows {
...
func GroupByOperator(rows []ParsedRow) []GroupedRows {

	// group by operator, ordering each group by start time ASC, end time ASC

	grouped := make([]*GroupedRows, 0)
	index := make(map[string]*GroupedRows)
	for _, activity := range rows {
		key := activity.Operator.Code
